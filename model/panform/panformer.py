...
class CrossSwinTransformer(nn.Module):
    def __init__(self, ms_chans=4, n_feats=32, n_heads=4, head_dim=16, win_size=4,
        ...
        #     conv3x3(n_feats * len(cat_feat), n_feats),
...
def test_net():
    ms = torch.randn([20, 4, 32, 32]).to('cuda')
    pan = torch.randn([20, 1, 128, 128]).to('cuda')
    # CT_4 = CT_transform(4)
    # CT_1 = CT_transform(1)
    ...
    net = CrossSwinTransformer().to('cuda')

    import thop
    flops, params = thop.profile(net, inputs=(ms, pan))
    print('flops:', flops)
    print('params:', params)
    y = net(ms, pan)
    print(y.shape)
