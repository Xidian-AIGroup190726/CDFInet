...
class WindowAttention(nn.Module):
    def __init__(self, dim, heads, head_dim, shifted, window_size, relative_pos_embedding, cross_attn):
        ...
        self.a = nn.Linear(dim, inner_dim, bias=False)
    ...
    def forward(self, x, y=None, dir=None):
        if dir is not None:
            factor = int(dir.shape[2] / x.shape[2])
            dir = dir.permute(0, 2, 3, 1)[:, ::factor, ::factor, :]
        if self.shifted:
        ...
        if not self.cross_attn:
            ...
            qkv = (qk[0], qk[1], self.to_v(y))
        a = self.a(x[:, ::2, ::2, :])

        nw_h = n_h // self.window_size
        nw_w = n_w // self.window_size
        aw_h = n_h // (self.window_size * 2)
        aw_w = n_w // (self.window_size * 2)

        q, k, v = map(
            lambda t: rearrange(t, 'b (nw_h w_h) (nw_w w_w) (h d) -> b h (nw_h nw_w) (w_h w_w) d',
                                h=h, w_h=self.window_size, w_w=self.window_size), qkv)
        a = rearrange(a, 'b (aw_h w_h) (aw_w w_w) (h d) -> b h (aw_h aw_w) (w_h w_w) d',
                                h=h, w_h=self.window_size, w_w=self.window_size)
        dir = rearrange(dir, 'b (nw_h w_h) (nw_w w_w) (h d) -> b h (nw_h nw_w) (w_h w_w) d',
                                h=h, w_h=self.window_size, w_w=self.window_size) if dir is not None else None

        if dir is not None:
            v = einsum('b h w i d, b h w i e -> b h w i e', dir, v)
        ...
        dots1 = einsum('b h i w d, b h j w d -> b h i j d', q, a) * self.scale
        dots2 = einsum('b h i w d, b h j w d -> b h i w j', k, a) * self.scale

        dots = einsum('b h w d i, b h w j d -> b h w i j', dots1, dots2) * self.scale
...
class SwinBlock(nn.Module):
    ...
    def forward(self, x, y=None, dir=None):
        x = self.attention_block(x, y=y, dir=dir)
        x = self.mlp_block(x)
        return x
...
class SwinModule(nn.Module):
    ...
    def forward(self, x, y=None, dir=None):
        if y is None:
            ...
            for regular_block, shifted_block in self.layers:
                x = regular_block(x, y, dir)
                x = shifted_block(x, y, dir)
            return x.permute(0, 3, 1, 2)
